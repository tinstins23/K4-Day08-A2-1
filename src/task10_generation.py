...
# LLM_MODEL: chọn model ":free" trên OpenRouter (không tính phí, phù hợp bài lab
# chưa có credit trả phí). "openai/gpt-oss-20b:free" — model open-weight 21B (Apache
# 2.0) của OpenAI, hỗ trợ tốt structured output + instruction-following tiếng Việt,
# 131K context (dư sức chứa context + citation của bài lab này). Nếu model bị
# rate-limit/delisted (free model trên OpenRouter hay đổi), thử "google/gemma-4-26b-a4b-it:free"
# hoặc "nvidia/nemotron-3-super-120b-a12b:free" làm phương án dự phòng.
...
def reorder_for_llm(chunks: list[dict]) -> list[dict]:
    ...
    front = chunks[::2]   # index 0, 2, 4, ... -> đặt ở đầu (giữ nguyên thứ tự score)
    back = chunks[1::2]   # index 1, 3, 5, ...  -> đặt ở cuối, đảo ngược
    return front + back[::-1]
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not api_key:
        raise RuntimeError(
            "Thiếu OPENROUTER_API_KEY (hoặc OPENAI_API_KEY). Điền vào file .env — "
